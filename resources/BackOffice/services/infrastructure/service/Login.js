...
export default class LoginService {
    ...
    async SignInRequest( payload ) {
        try {
            ...
            let response = await HTTPService.postRequest( 'https://jsonplaceholder.typicode.cm/psts', payload );
            console.log(response, 'response');
        } catch ( exception ) {
            this.$vm.displayNotification(( '' || 'متاسفانه مشکلی پیش آمده است.' ), {
                type: 'error',
                duration: 4000
            })
        } finally {
    ...
    async _onClickSubmitButton( payload ) {
        try {
            ...
            let data = {
                user: toEnglishDigits( this.$vm.form.username.value ),
