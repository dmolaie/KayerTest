        ...
        <div class="header__nav_container container flex items-center">
            @foreach($menus as $menu)
                <div class="header__nav_menu relative cursor-pointer text-nowrap text-black font-xs-bold">
                    <a class="text-nowrap text-black font-xs-bold" href="{{ is_null($menu->parent_id) ?
            ...
            <div class="header__user header__user--{{auth()->check() ? 'active' : 'toggle'}} relative flex items-center">
                ...
                <div class="header__user--icon relative flex items-center {{ !auth()->check() ? 'login-box border border-solid rounded' : '' }}">
                    ...
                    @if( auth()->check() )
                        <div class="header__dropdown absolute bg-white border border-solid rounded-10 z-10">
                            @if( !(config('role.roles.client.name')==auth()->user()->roles->sortBy('priority')->pluck('type')->toArray()[0]))
